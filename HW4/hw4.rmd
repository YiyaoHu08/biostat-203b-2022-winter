...
Model 1: GLM (thirty_day_mort ~ gender + age+ ethnicity + marital_status + labvaluenum +vitalvaluenum)
```{r}
...
model1 <- glm(thirty_day_mort ~ gender + age+ ethnicity + marital_status + 
                   labvaluenum +vitalvaluenum, data = training_set, 
                  family = 'binomial')
...
possibility <- predict(model1, newx=new_x,  type="response") # predicted values
predicted <- ifelse(possibility > 0.5, 1, 0)

# Model accuracy
observed <- training_set$thirty_day_mort
...
# Model accuracy
observed.classes <- modeldata2$thirty_day_mort
