...
Model 2: Fit the lasso penalized regression model:
```{r}
library(glmnet)
set.seed(345) 
...
cv.lasso <- cv.glmnet(x, y, alpha = 1, family = "binomial")
 ...
 predict( glmnet(x,y), s=ccv.lasso$lambda.min )
...
The two models did the same well as for predictive accuracy.
...
The plot shows at a threshold of 0.095, the corresponding sensitivity and specificity are 70.9% and 57% respectively, which means the
model has a just so-so performance. In adtion, the error rate would be pretty high at this cut-off. So I adjusted the cut-off to 0.23 when using the fitted model to predict for the testing data set.
```{r}
...
The plot shows at a threshold of 0.091, the corresponding sensitivity and specificity are 74% and 54% respectively, which means the
model has a just so-so performance. In addition, the error rate would be pretty high at this cut-off. So I adjusted the cut-off to 0.23 when using the fitted model to predict for the testing data set. The final predictive accuracy is 88%.
