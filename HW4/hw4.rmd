...
model1 <- glm(thirty_day_mort ~ gender + age+ ethnicity + marital_status + 
...
3. Compare model prediction performance on the set.

The two models did the same well as for predictive accuracy.

Model 1: 
Overall accuracy of Prediction: 0.897
...
predicted <- ifelse(possibility > 0.5, 1, 0)
...
measure <- measureit(score = observed, class = predicted,
...
library(ROCit)
roc_empirical <- rocit(score = observed, class = predicted) 
summary(roc_empirical)

message("Number of positive responses used: ", roc_empirical$pos_count)
#> Number of positive responses used: 13
message("Number of negative responses used: ", roc_empirical$neg_count)
#> Number of negative responses used: 42457
message("The AUC: ", roc_empirical$AUC)
#> The AUC:0.602193893912574
...
probabilities <- model2 %>% predict(newx = x.test, type = 'response')
predicted_classes <- as.vector(ifelse(probabilities > 0.5, 1, 0))
class(predicted)
...
# Performance metrics of binary classifier
measure <- measureit(score = observed_classes, class = predicted_classes,
                     measure = c("ACC", "MIS", "SENS", "SPEC"))
print(measure)


library(ROCit)
roc_empirical <- rocit(score = observed_classes, class = predicted_classes) 
summary(roc_empirical)
