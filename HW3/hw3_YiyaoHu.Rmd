---
title: "Biostat 203B Homework 3_YiyaoHu"
author: "Yiyao Hu @ UCLA"
...
```{r}
# # TODO
admission_tble <- tbl(con, "admissions") %>%
  show_query() %>%
  print(width = Inf)

admissions_tble <- admission_tble %>%
...
```{r}
# # TODO
labevents_tble_2 <-labevents_tble %>%
  mutate(labitemid = itemid,
         labvaluenum = valuenum) %>%
  select(-c(itemid, valuenum))

chartevents_tble_2 <-chartevents_tble %>%
...
icu_cohort <- icustays_tble %>% 
  ...
  left_join(labevents_tble_2, by = "subject_id") %>%
  left_join(chartevents_tble_2, by = "subject_id") %>%
  mutate(age_hadm = anchor_age + year(admittime) - anchor_year) %>%
  filter(age_hadm >= 18) %>%
  mutate(alive_days_before_death = day(dod) - day(admittime)) %>%
  mutate(thirty_day_mort = alive_days_before_death < 30) %>%
  print(width = Inf) %>%
