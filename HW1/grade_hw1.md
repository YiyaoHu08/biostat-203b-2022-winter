...
-   Are the materials (files and instructions) submitted to the `main` branch sufficient for reproducing all the results? Just click the `knit` button will produce the final `html` on teaching server? 
    ...
    We would like to actually run most of our code. Particularly, we want to run middle.sh, so you need to include that file. We want lines 170-172 and 213-215 to be `eval = TRUE` so we reproduce the full output. `-3.`
