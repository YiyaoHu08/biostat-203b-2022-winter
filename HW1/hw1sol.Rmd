...
```
Why these data files are distributed as `.csv.gz` files instead of `.csv` is CSV. GZ is a CSV file compressed with gzip utility. 
...
Number of rows are in this data file?
    ```{bash, eval=T}
...
--no-clobber
    ...
    do
      echo $char:grep pg42671.txt| head -10
      # some commands here
    done
...
2. What's the difference between the following two commands?
    ...
    ```
**Solution**:
> directs output from one commend to a file;
>> appends output from one commend to a file.
